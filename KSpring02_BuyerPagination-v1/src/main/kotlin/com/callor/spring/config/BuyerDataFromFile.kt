...
class BuyerDataFromFile(val buyerDao:BuyerRepository) {
    ...
    *.properties 또는 *.yml 파일에 설정된 속성(com.callor.spring.student-file) 을
     ...
     */
    @Value("\${com.callor.spring.studentFile}")
...
//        val file = ClassPathResource(dataFile)
        ...
        // ClassPathResource() 함수를 사용하여 Resources 폴더에 저장된 정보를 가져오면
        // 다음과 같은 속성값을 취할 수 있다
        logger().debug("객체 {}", file?.file.toString())
        logger().debug("이름 {}", file?.filename.toString())
        logger().debug("InputStream {}", file?.inputStream.toString())
        logger().debug("파일저장경로 객체 {}", file?.path.toString())
        ...
        Files 클래스의 readAllLines() 함수에 Path 객체를 만든다
        ...
        val managerList = fileList.map{
            val linse = it.split(":")
//            {이름 : "홍길동", 전화번호 : "010-1111-2222" } 형식의 데이터 생성
            mapOf(이름 to linse[이름], 전화번호 to linse[전화번호])
        }
